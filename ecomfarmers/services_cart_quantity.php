...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['services_id'], $_POST['new_quantity'])) {
        ...
        $product = getProductDetails($servicesID);

        if ($product) {
            // Update the cart item quantity in the session
            if (!empty($_SESSION['cart'][$servicesID])) {
                $_SESSION['cart'][$servicesID]['quantity'] = $newQuantity;
            }
...
function getProductDetails($servicesID) {
    ...
    return isset($products[$servicesID]) ? $products[$servicesID] : null;
}
?>